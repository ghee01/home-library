...
#             @dataclass 데코레이터를 넣으면 자동으로 __init__ 생성
from dataclasses import dataclass
from pathlib import Path
import io   # 메모리 안에서 파일처럼 다룰 수 있도록 하는 라이브러리
import uuid # 랜덤 문자열 생성 (중복되는 파일이 안 생기도록 해준다)
...
class ServiceResult:
    ...
    API 라우터 → 실패 시, HTTPException 에러 상태 코드로 응답
    HTML 라우터 → 실패 시, 에러 화면이 아니라 "이미 등록된 책입니다"와 같은 안내문구가 나와야 한다

    status: str → 결과 상태
        - ok : 정상 등록/조회 성공
        - invalid_isbn : 잘못 입력된 isbn
        - duplicate : 이미 등록된 책
...
def lookup_book_service(isbn: str, db: Session) -> ServiceResult:
    """
    ISBN만으로 국립중앙도서관 서지정보를 조회해서 책을 등록하는 함수 (main.py의 lookup_book 함수를 옮겼다)
    """
    ...
    existing_book = _find_by_isbn(db, isbn)
    if existing_book:
        # 참이라면 실제 Book 객체 존재 → 이미 있는 책
    ...
    db.add(book)        # 저장 대기열에 올림 → 아직 DB에 실제로 쓰이지 X
    db.commit()         # 대기열에 있는 책을 DB에 확정 저장 → SQL INSERT 실행
    db.refresh(book)    # DB가 자동 생성한 값(예: id, created_at)을 book 객체에 다시 채워 넣음
...
def register_book_service(isbn: str, raw_image: bytes, original_filename: str, db: Session) -> ServiceResult:
    """
    ISBN + 책 표지 사진 함께 등록하는 함수 (main.py의 register_book 함수의 로직을 따왔다)
    ...
    path.write_bytes(raw_image)     # 실제로 디스크에 이미지 파일 저장
    ...
    if metadata:
        ...
        # 서지정보를 못찾았어도 책 표지 사진과 ISBN이 있다면 일단 등록 (수동 등록)
    ...
    book = Book(
        ...
        author=author,
        publisher=publisher,
        cover_path=str(path),   # DB는 Path 객체같은 건 모른다. 문자열만 이해한다.
