...
기존 JSON API 라우터(/books/lookup, /books/register, /books) 응답 형태 그대로 두고,
...
# StaticFiles : 해당 폴더의 파일들을 그대로 웹 주소로 접근 가능하게 공개하는 기능
from fastapi.staticfiles import StaticFiles

# Jinja2Templates : templates 폴더 안의 .html 파일에 파이썬 데이터를 채워서 완성된 HTML 페이지로 만들어준다
...
# services 폴더 안 book_service.py의 lookup_book_service, register_book_service 가져오기
...
# 주소를 직접 문자열로 '/static/style.css'라고 하드코딩 하지 않는 이유
#   - 나중에 마운트 경로가 바뀌어도 템플릿을 일일이 안 고쳐도 됨
# ---------------------------------------------------------------------

app.mount('/uploads', StaticFiles(directory=UPLOAD_DIR), name='uploads')
app.mount('/static', StaticFiles(directory='static'), name='static')

templates = Jinja2Templates(directory='templates')  # html 찾아라
...
def lookup_book(isbn: str, db: Session = Depends(get_db)):
    """
    ISBN으로 DB에서 찾아서 결과 확인
    """
    result = lookup_book_service(isbn, db)

    if result.status == 'invalid_isbn':
        # 상태코드 422 → Unprocessable Entity (요청은 이해했지만 값이 유효하지 않다)
    ...
    if result.status == 'not_found':
        # 상태코드 404 → Not Found (요청한 대상을 찾을 수 없다. 서지정보 없다.)
...
def register_book(
    ...
    ISBN + 표지 사진을 DB에 등록
    """
    # image.file.read() : 업로드 파일 내부의 실제 파일 객체를 열어서 내용을 전부 읽어 bytes(0과 1의 나열)로 가져온다
    raw = image.file.read()
    ...
    if result.status == 'invalid_image':
        # 상태코드 415 → Unsupported Media Type (파일 형식이 이미지가 아니다)
...
def ui_lookup_form(request: Request):
    ...
    request: Request
        - Jinja2Templates가 화면을 만들 때 반드시 필요로 하는 매개변수
        - 내부적으로 어떤 요청이 어떤 서버/포트로 왔는지 등을 알아야 url_for() 같은 기능이 동작한다
    templates.TemplateResponse(...)
        - context에 담긴 데이터를 채워 넣은 뒤, 완성된 html을 브라우저에 응답으로 보낸다
    context={}
        - 맨 처음 화면 진입 시 조회 결과가 없으므로
    """
...
def shelf(request: Request, db: Session=Depends(get_db)):
    ...
    """
    # .order_by(Book.created_at.desc()) : 등록된 시각 기준으로 최신순(내림차순) 정렬
